...
invoiceController.createInvoice = async (req, res) => {
  ...
  // Convert items to a JSON string if it's not already a string
  // This ensures the value is correctly formatted for a JSON/JSONB column in PostgreSQL.
  ...
  try {
    const result = await pool.query(
      ...
      [admin_id, customer_id, amount, status, due_date, itemsStr, payment_method]
    );
...
invoiceController.updateInvoice = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [amount, status, due_date, itemsStr, payment_method, id]
    );
